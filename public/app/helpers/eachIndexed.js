...
(function (global) {
    ...
    require(deps, function (Ember) {
		Ember.Handlebars.registerHelper('eachIndexed', function (path, options) {
		  ...
		  // Process arguments (either #eachIndexed bar, or #eachIndexed foo in bar)
		  if (arguments.length === 4) {
			...
			Ember.assert(arguments[0] +' is a reserved word in #eachIndexed', $.inArray(arguments[0], ['index', 'index1', 'first', 'last', 'even', 'odd']));
		  ...
		  function eachFn(){
			var keywords = arguments[1].data.keywords,
				...
				list = view.get('content') || [];
		 
			// Set indexes
			keywords['index'] = index;
			keywords['index1'] = index + 1;
			keywords['first'] = (index === 0);
			keywords['last'] = (index + 1 === list.length);
